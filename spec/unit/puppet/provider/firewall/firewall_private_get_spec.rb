...
RSpec.describe Puppet::Provider::Firewall::Firewall do
  describe 'Private Methods - Get' do
    ...
    describe 'self.get_rules' do
      context 'when iptables-save output contains * in a rule comment' do
        let(:iptables_output) do
          <<~IPTABLES
            ...
            *filter
            ...
            :FORWARD ACCEPT [0:0]
            :OUTPUT ACCEPT [0:0]
            -A INPUT -p tcp -m comment --comment "001 allow * tcp"
            -A OUTPUT -p tcp -m comment --comment "002 test rule"
            COMMIT
            *raw
            :PREROUTING ACCEPT [0:0]
            :OUTPUT ACCEPT [0:0]
            -A PREROUTING -p udp -m comment --comment "003 raw rule"
            COMMIT
          IPTABLES
        end

        it 'parses rules from all tables without being broken by * in a comment' do
