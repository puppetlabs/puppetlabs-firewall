...
ARGS_TO_HASH = {
  ...
  'match_mark' => {
    :line => '-A INPUT -p tcp -m comment --comment "066 REJECT connlimit_above 10 with mask 32 and mark matches" -m connlimit --connlimit-above 10 --connlimit-mask 32 -m mark --mark 0x1 -j REJECT --reject-with icmp-port-unreachable',
    :table => 'filter',
    :params => {
      :proto           => 'tcp',
      :connlimit_above => '10',
...
HASH_TO_ARGS = {
 ...
 'source_destination_ipv6_netmask' => {
  ...
  'match_mark' => {
    ...
    :args => ["-t", :filter, "-p", :tcp, "-m", "comment", "--comment", "066 REJECT connlimit_above 10 with mask 32 and mark matches", "-j", "REJECT", "-m", "connlimit", "--connlimit-above", "10", "--connlimit-mask", "32", "-m", "mark", "--mark", "0x1"],
  },
}
